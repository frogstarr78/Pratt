...
#      Pratt.connect self.env
  ...
  def graph
    ...
    projects.each do |proj| 
      puts "%1$*4$s%2$s %3$s"% [proj.name, (color ? 'â®' : '|'), Pratt.totals(proj.time_spent(scale, when_to), color && true), max] if show_all or ( !show_all and proj.time_spent(scale, when_to) != 0.0 )
    end
    puts (color ? 'Â·' : '-')*60
    puts [
...
expect #{app.pid.to_s.magenta} Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â·Â· â#{daemonized? ? 'OK'.green : 'Oops'.red}â
  ...
  def raw
    ...
    if project = Project.find_by_name( raw_conditions )
      project.whences.all(:order => "id ASC").each do |whence| 
        ...
        str   = "%#{max}.#{max}s â%s"% [("%s"%whence.project.name), whence.start_at.strftime(FMT).send(color)]
        str  += "Â­%sâ %0.2f min"% [whence.end_at.strftime(FMT).send(color), (whence.end_at-whence.start_at)/60] if whence.end_at
        puts str
      end
    else
      case raw_conditions
